# Hotel funnel report — fujiseat.com

_Last 28 days · generated 2026-08-12_

## 1. Hotel pages — views vs affiliate clicks
Median affiliate-click rate across hotel pages: **8.0%**

| Page | Views | Affiliate clicks | Click rate | Flag |
|---|--:|--:|--:|---|
| /areas-to-stay/asakusa-vs-ueno | 50 | 4 | 8.0% |  |
| /areas-to-stay/tokyo-stay-area-index | 36 | 0 | 0.0% |  |

## 2. Tokyo hotel-area finder funnel
| Step | Count | vs start |
|---|--:|--:|
| finder_start | 3 | 100.0% |
| finder_step_answered | 10 | 333.3% |
| finder_results_view | 3 | 100.0% |
| finder_area_details_click | 0 | 0.0% |
| finder_result_hotel_page | 0 | 0.0% |

## 3. CTA impressions vs clicks by month (180d)
Judge trends on the **organic** column — direct bot traffic distorts the raw totals.

| Month | cta_view | click | Conversion | Organic conversion |
|---|--:|--:|--:|--:|
| 202604 | 0 | 3 | — | — |
| 202605 | 0 | 52 | — | — |
| 202606 | 0 | 30 | — | — |
| 202607 | 517 | 46 | 8.9% | 6.0% |
| 202608 | 320 | 11 | 3.4% | 3.4% |

## 4. Affiliate clicks by placement
| Placement | Clicks |
|---|--:|
| guide_quick_answer | 12 |
| home_seat_result | 8 |
| comparison_area_card | 4 |
| plan_trip_activity_cards | 4 |
| comparison_quick_recommendation | 2 |
| guide_how_to_book | 2 |
| plan_trip_arrival_cards | 1 |
| train_signs_checklist | 1 |

## 5. Search Console — hotel-intent quick wins
Hotel-intent queries ranking 5–20 with real impressions (push these onto page 1):

| Query | Impressions | Clicks | CTR | Avg position |
|---|--:|--:|--:|--:|
_No hotel-intent queries in the 5–20 band this period._

## 6. Suggested next steps (heuristic)
1. **CTA:** `/areas-to-stay/asakusa-vs-ueno` gets 50 views but only 8.0% affiliate-click rate — add/raise a clearer hotel CTA (sticky bar, in-content provider button) and re-check.
2. **Finder → hotel:** few results lead to an area/hotel click — make result cards more clickable and surface a hotel CTA right on the results.

---
_Read-only snapshot. For a deeper read, paste this file into Claude and ask for prioritized funnel fixes._
