...
このプロジェクトで起きた失敗は、ほぼ全て「データを見る前に動いた」ことが原因。

- **CTA・商材を配置する前に、そのページの表示回数と順位を必ず確認する。**
  文脈が合っていても、流入が無いページに置いた商材は売れない。実例: 順位36位・四半期112表示のページに新商材12件を配置し、4日間の表示ゼロ。
- **ページを削除・301する前に、順位を確認する。**
  順位20位台でクリック0は当然であって、削除の理由にならない。実例: narita-to-shibuya を「クリック0」で削除したが、順位26位で158表示あり、内容を厚くすれば拾えた可能性がある。
- **「壊れている」と結論する前に、自分のコードを疑う。**
  実例: 「非英語版guideの転換率0%」は、CTAが `page_path` に `/guide` をハードコードしていたための計測ミス。実際は英語版と同水準だった。
- **新しい領域への拡張を提案する前に、その領域のクエリ実績を GSC で確認する。**
  実例: 「新幹線チケットの買い方」領域を提案したが、実測すると39クエリ・298表示しか無く、足がかりが存在しなかった。
- 判断を訂正するときは、何をどう間違えたかを明示してから訂正する。

### 分析
- 判断は**自然検索のみ**で行う。中国からの直接流入（全体の約35%、滞在5秒）が指標を歪める。
- GSC のページ別クエリは `dimensionFilterGroups` を使う。`dimensionFilter` はフィルタが効かない。
- GSC のクエリ別データは全表示の約8%しか見えない（残りは匿名化）。「クエリに出ていない＝需要が無い」とは限らない。
- アフィリエイトクリックは90日で約106件と母数が小さい。1〜2件差を根拠にしない。
- GA4 より提携先（Klook / Travelpayouts）の実データを優先する。
- **収益の制約は流入量**。1日20訪問では、収益化を完璧にしても月¥2,500程度が上限。毎日発生させるには1日100〜120訪問が必要。ボタンの改善で埋まる差ではない。
