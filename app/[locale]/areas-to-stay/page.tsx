  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-[#106b43]">
            {page.quickRec.area} recommended
          </p>
          <h2 className="mt-1 text-base font-semibold text-slate-950 group-hover:text-[#106b43]">
            {page.title}
          </h2>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {page.areas.map((area) => (
          <span
            key={area.name}
            className="rounded-full border border-[#9fd7bd] bg-[#f0fbf6] px-2.5 py-0.5 text-[10px] font-semibold text-[#106b43]"
          >
            {area.name}
          </span>
        ))}
